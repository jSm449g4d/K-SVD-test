...
im2=cv2.imread("./manner_door_open_ashi.png")
im2 = cv2.cvtColor(im2, cv2.COLOR_BGR2GRAY)#cv2.COLOR_BGR2RGB
im2 = cv2.resize(im2, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)/256.0
im2=np.clip(im2, 0.001, 0.999)
plt.title("origin2");plt.imshow(im2,vmin=0.0, vmax=1.0);plt.show()

ksvd = KSVD(n_components = 12, transform_n_nonzero_coefs = None ,n_jobs=12)
ksvd.fit(im2.reshape(-1,64).astype(np.float64))
...
X = ksvd.transform(im2.reshape(-1,64).astype(np.float64))
plt.title("X_2")
plt.imshow(X,vmin=0.0, vmax=1.0)
plt.show()
...
print(compare_psnr(im2, _y))
print(compare_ssim(im2, _y))
